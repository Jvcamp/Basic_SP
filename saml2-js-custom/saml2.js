...
create_metadata = function(entity_id, assert_endpoint, logout_endpoint, signing_certificates, encryption_certificates) {
...
create_logout_request = function(issuer, name_id, name_id_format, session_index, destination) {
  var id, xml;
  id = '_' + crypto.randomBytes(21).toString('hex');
  xml = xmlbuilder.create({
    'samlp:LogoutRequest': {
...
module.exports.ServiceProvider = ServiceProvider = (function() {
  function ServiceProvider(options) {
    ...
    this.entity_id = options.entity_id, this.private_key = options.private_key, this.certificate = options.certificate, this.assert_endpoint = options.assert_endpoint, this.logout_endpoint = options.logout_endpoint, this.alt_private_keys = options.alt_private_keys, this.alt_certs = options.alt_certs;
        ...
        return function(response_buffer, cb_wf) {
          ...
          switch (false) {
            ...
            case saml_response.getElementsByTagNameNS(XMLNS.SAMLP, 'LogoutResponse').length !== 1:
			  ...
			  //start joren
			  if(saml_response.getElementsByTagNameNS(XMLNS.SAMLP, 'StatusCode')[0].getAttribute('Value') == 'urn:oasis:names:tc:SAML:2.0:status:Success'){
  ...
  ServiceProvider.prototype.create_logout_request_url = function(identity_provider, options, cb) {
    ...
    options = set_option_defaults(options, identity_provider.shared_options, this.shared_options);
    _ref = create_logout_request(this.entity_id, options.name_id, options.nameid_format, options.session_index, identity_provider.sso_logout_url), id = _ref.id, xml = _ref.xml;
    return zlib.deflateRaw(xml, (function(_this) {
      return function(err, deflated) {
        var query, uri;
        if (err != null) {
  ...
  ServiceProvider.prototype.create_metadata = function() {
    ...
    return create_metadata(this.entity_id, this.assert_endpoint,this.logout_endpoint, certs, certs);
  };
